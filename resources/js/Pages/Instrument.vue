...
<Layout :user="$page.props.user">
    ...
    <template #subNav>
        <div class="w-full py-1.5 flex gap-1.5 sticky top-14 subnavBg slate-900 dark:border-primaryDarkBorder border-t border-slate-200 px-2.5 z-20">
            <div class="relative">
                <CreateButton @click="openCreateAs" :disabled="$page.props.createAs > 3" >
                    <AddIcon />
                    <div class="ml-2 mr-1 whitespace-nowrap max-sm:hidden" v-if="$page.props.createAs === 0">Instrument Name</div>
                    <div class="ml-2 mr-1 whitespace-nowrap max-sm:hidden" v-if="$page.props.createAs === 1">Create Instrument</div>
    ...
    <div v-if="$page.props.current && $page.props.current.category === null" class="mt-3">
  <!-- When category is null, show the instruments -->
...
<div v-else-if="$page.props.current && $page.props.current.category === 'inst'" class="mt-3">
  <!-- When category is 'inst', show the level select dropdown and filter instruments -->
  <label for="level-select" class="block text-sm font-medium text-gray-700">Select Level</label>
  <select 
    id="level-select" 
...
<div v-else class="mt-3">
  <!-- When no specific category is selected, show the default instruments -->
  <div class="grid grid-cols-[repeat(auto-fill,minmax(17rem,1fr))] gap-3 mt-3" v-if="$page.props.createAs <= 1">
    <InstrumentCard 
      :instrument="instrument" 
      :selected="storeInstrument.selected"
...
const filteredInstruments = computed(() => {
  if (!selectedLevel.value) {
    return instruments.value.filter(instrument => instrument.title === "Level 1");// If no level is selected, show all instruments
  }
  // Filter instruments where the title matches the selected level
  return instruments.value.filter(instrument => instrument.title === selectedLevel.value);
})
const route = (id, route, current) => {
    Inertia.get('/program/instrument/inst/'+id)
    storeInstrument.selected = null
}

const roteTo = (instRoute) => {
    Inertia.get('/program/instrument/inst/'+instRoute.id)
    storeInstrument.selected = null
}
...
const backRoute = () => {
    ...
    {
        Inertia.get('/program/instrument')
    }else{
        Inertia.get('/program/instrument/inst/'+props.crumbs.at(0).parent)
    }
    storeInstrument.selected = null
}
...
const deleteInstrument = () => {
    Inertia.post('/program/instrument/delete/',{
