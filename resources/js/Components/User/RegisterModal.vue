<template>
    ...
    <select id="Institute" class="formInput mt-2" v-model="register.institute">
        ...
        <!-- Iterate institutes and ensure the first item appears first -->
        <option :value="institute.id" v-for="(institute, index) in institutes" :key="institute.id">
            ...
            <div class="mt-7 mb-3">
                ...
                <Submit width="w-32 ml-auto"  class="!bg-green-500 !text-white !hover:bg-blue-600 px-4 py-2 rounded" :disabled="!register.designation">
...
const deleteUser = async () => {
    if (confirm('Are you sure you want to delete this user?')) {
        try {
            await axios.delete(`/user/delete/${props.user.id}`);
...
.then(res => {
    ...
    // Set default institute value to the first item if no institute is set
    if (institutes.value.length > 0 && !register.institute) {
        register.institute = institutes.value[0].id;
    }

    if (props.isEdit) {
        register.auth = res[3].data.auth ? parseInt(res[3].data.auth) : null;
        register.institute = res[3].data.instituteId ? parseInt(res[3].data.instituteId) : register.institute;
