<template>
    <div class="">
        <div class="flex rounded items-center cursor-pointer pr-2" :class="[showSubs?'bg-earistRed/20 text-earistRed dark:text-white/60 dark:bg-earistRed/30':'bg-gray-100 hover:bg-gray-200 dark:bg-tertiaryDarkBg hover:dark:bg-white/20']" 
        @click.self="ratesInst(insts.id)">
            ...
            <button class="ml-2 hover:bg-earistRed hover:text-white rounded relative group
            " @click="reCalculates(insts.id)">
                <SyncIcon/>
                <div class="absolute text-xs whitespace-nowrap group-hover:block bg-earistRed/20 text-earistRed right-5 top-4 p-px rounded hidden z-20">Re-calculate rate</div>
            </button>
        </div>
        <div v-if="showSubs">
            <div 
