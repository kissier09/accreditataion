    ...
    <div class="w-full h-[80vh] flex mb-2 gap-2.5 analytics">
        ...
        <div class="w-full rounded subnavBg grow h-full flex-col p-2 pt-0 analytics" :class="[clusterContainer.ViewCluster?'flex':'hidden']">
            <div class="h-12 flex items-center shrink-0 gap-1">
                <button 
                    class="rounded w-fit px-2 transition_300 hover:bg-earistRed/20 hover:text-earistRed py-1.5 dark:hover:bg-earistRed dark:text-white/80" 
                    @click="displayCluster('best')"
                    :class="[clustered.val == 'best'? 'bg-earistRed/20 text-earistRed dark:bg-earistRed':'']"
                >Best practices</button>
                <button 
                    class="rounded w-fit px-2 transition_300 hover:bg-earistRed/20 hover:text-earistRed py-1.5 dark:hover:bg-earistRed dark:text-white/80" 
                    @click="displayCluster('avg')"
                    :class="[clustered.val == 'avg'? 'bg-earistRed/20 text-earistRed dark:bg-earistRed':'']"
                >Average</button>
                <button 
                    class="rounded w-fit px-2 transition_300 hover:bg-earistRed/20 hover:text-earistRed py-1.5 dark:hover:bg-earistRed dark:text-white/80" 
                    @click="displayCluster('low')"
                    :class="[clustered.val == 'low'? 'bg-earistRed/20 text-earistRed dark:bg-earistRed':'']"
                >Needs improvement</button>

                <button 
                    v-if="clusterContainer.ViewCluster && clusterContainer.hasClustered"
                    class="flex items-center justify-center w-6 h-6 rounded-full bg-earistRed/20 pl-px pt-px 
                    text-earistRed hover:bg-earistRed hover:text-white transition_300 ml-auto" @click="clusterContainer.ViewCluster = false, clusterContainer.hasClustered = false">
                    <XIcon/>
                </button>
            </div>
