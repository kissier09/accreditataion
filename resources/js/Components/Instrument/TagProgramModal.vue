            ...
            <div class="mt-7 max-w-[29rem]">
                ...
                <form @submit.prevent="submit">
                    ...
                    <div class="mt-7 mb-3 flex gap-3 justify-end">
                        ...
                        <button type="button" class="w-32 bg-red-500 text-white py-2 rounded-md" @click="deleteTag">
                            Delete
                        </button>
                    </div>
                </form>
...
const deleteTag = () => {
    if (selectedProgram.value) {
        console.log("Level ID:", props.level.id);
        axios.delete(`/accreditation/delete_tag_program?level_id=${props.level.id}`)
        .then(res => {
            if (res.data.success) {
                alert("Program tag deleted successfully!");
                emits('update', []); // Notify parent component
                emits('handleClose'); // Close the modal
                tagForm.reset(); // Reset the form
                tagForm.clearErrors(); // Clear any errors
            } else {
                console.error("Failed to delete the program tag.");
            }
        })
        .catch(err => {
            console.error("Error deleting program tag:", err);
        });
    }
};
