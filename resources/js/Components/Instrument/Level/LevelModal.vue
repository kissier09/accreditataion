<template>
    <Modal @handle-close="$emit('handleClose')">
        <template #header>
            <span v-if="isEdit">Update Instrument</span>
            <span v-else>Create Instrument</span>
        </template>
        ...
        <form @submit.prevent="submit($page.props.current.id)" class="mt-9">
            <div>
                ...
                <input type="text" :class="[levelForm.errors.level_and_phase?'formInput_error':'formInput']"  placeholder="Level and phase" v-model="levelForm.level_and_phase" readonly>
            </div>
...
if(props.isEdit)
{
    ...
    levelForm.level_and_phase = 'LEVEL'  // default value for new entry
}
</script>