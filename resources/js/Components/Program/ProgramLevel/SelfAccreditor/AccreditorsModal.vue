                ...
                <div class="w-full overflow-y-auto p-1">
                    <div class="w-full overflow-y-auto p-1">
                        <div class="h-11 w-full hover:bg-gray-300/80 dark:hover:bg-white/10 rounded transition_300 cursor-pointer flex items-center px-3 gap-2.5 inter-font itemCard"
                            v-for="(user, index) in accreditors" :key="user.id">
                            {{ user.user }}
                            <button @click="deleteAccreditor(user.id)" class="ml-2 text-red-500">Delete</button>
                        </div>
                    </div>
                </div>
            </div>
...
const deleteAccreditor = (id) => {
    // Remove accreditor from the front-end
    accreditors.value = accreditors.value.filter(acc => acc.id !== id)

    // Send request to backend to remove accreditor
    axios.post('/program/remove/selfAccreditor', { id: id }).then(res => {
...
try {
    axios.post('/program/getassigned/selfAccreditor', {
        ...
        console.error('Error fetching assigned accreditors:', error)
    })
} catch (e) {
    console.log(e)
}
