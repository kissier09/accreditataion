<template>
    <div class="mx-auto z-30 lg:ml-[] min-h-[20rem] md:w-[30rem] max-sm:w-[26rem] subnavBg rounded-lg py-6 px-8 my-8 dark:shadow-[inset_0_1px_0_0_hsl(0deg_0%_100%_/_5%)] relative">
        <div class="w-full text-center pb-2 text-lg">
            <span class="font-semibold text-earistRed">Pre registration </span>
        </div>
        <SuccesReg v-if="successReg" />
        <form @submit.prevent="submit">
            <div class="mt-1">
                <label for="email">Email</label>
                <span class="errorMessage ml-2 inter-font" v-if="Register.errors.email">{{ Register.errors.email }}</span>
                <input 
                    type="text" 
            ...
            <div class="mb-5 mt-10">
                <Submit width="w-full">Register</Submit>
            </div>
        </form>
        <div class="border-t pt-6 border-slate-200 dark:border-primaryDarkBorder">
            ...
            <div class="mb-2"></div>
            <Link href="/index" class="pb-4 w-fit">Already have an account? 
