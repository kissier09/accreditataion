...
class AccreditorController extends Controller
{
    ...
    public function removeAccreditor(Request $request)
    {
        try {
            // Find and delete the accreditor from the database
            $accreditor = AreaSelfAccreditor::find(id: $request->id);
            $accreditor->delete();

            return response()->json(['message' => 'Accreditor removed successfully.'], 200);
        } catch (\Exception $e) {
            // Handle any errors that occur during the deletion process
            return response()->json(['error' => 'Failed to remove accreditor.'], 500);
        }
    }
}
