...
class AccreditationController extends Controller
{
    ...
    public function deleteTagProgram(Request $request)
{
    ...
    // Find and delete the tag program (implement your delete logic here)
    $tagProgram = accreditation::where('instrumentId', $request->level_id)
                            ->first();

    if ($tagProgram) {
        $tagProgram->delete(); // Delete the tag program
        return response()->json(['success' => true, 'message' => 'Program tag deleted']);
    } else {
        return response()->json(['success' => false, 'message' => 'Program tag not found'], 404);
    }
}
