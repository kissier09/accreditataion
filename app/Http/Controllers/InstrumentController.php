...
class InstrumentController extends Controller
{
    ...
    public function store(Request $request, $id = null)
    {
        if($request->createAs === 0)
        {
            ...
            $request->validate(['level_and_phase'=>['required']]);
                $exist = Instrument::where('title', operator: $request->level_and_phase)->where('parent', $request->parent)->first();
    ...
    function createArea(Request $request, $id = null)
    {
        ...
        if($id)
        {
            ...
            try{
                ...
                DB::transaction(function () use ($request) {
                    $instrumentsForLevels = Instrument::where('id', $request->parent)->get();
                    $instrumentsForAllLevels = Instrument::whereIn('parent', $instrumentsForLevels->pluck('parent'))->get();
                
                    foreach ($instrumentsForAllLevels as $instrument) {
                        $area = Instrument::create([
                            'title' => $request->area, 
                            'parent' => $instrument->id,
                            'description' => $request->area_label,
                            'category'=>'area',
                            'binst'=>$instrument->binst,
                            'blevel'=> $request->parent,
                        ]);
            ...
            */
            ...
            try{
                DB::transaction(function () use ($request, $indicators) {
                    $instrumentsForArea = Instrument::where('id', $request->parent)->get();
                    $instrumentsForAllArea = Instrument::whereIn('title', $instrumentsForArea->pluck('title'))->get();
                
                    foreach ($instrumentsForAllArea as $instrument) {
        ...
        if($id)
        {
            ...
            try{
                ...
                DB::transaction(function () use ($request, $evidences) {
                    $instrumentsForParam = Instrument::where('id', $request->parent)->get();
                    $instrumentsForAllParam = Instrument::whereIn('title', $instrumentsForParam->pluck('title'))->get();
    ...
    public function delete(Request $request)
    {
        ...
        if ($instrument) {
            ...
            // Return an Inertia response (redirect or render)
