...
class ProgramController extends Controller
{
    ...
    public function destroy($id)
    {
        ...
        if (!$program) {
            return redirect()->route('program_indx')
                ->with('error', 'Program not found.');
        }

        // Check if any related records exist in the accreditations table
        $accreditations = Accreditation::where('programId', $id)->get();

        // Delete related accreditations if any
        foreach ($accreditations as $accreditation) {
            $area_assigns = AreaAssign::where('levelId', $accreditation->id)->get();
            foreach ($area_assigns as $area_assign) {
            $area_assign->delete();
            }
            $areaselfassigns = AreaSelfAccreditor::where('accredlvl', $accreditation->id)->get();
            foreach ($areaselfassigns as $areaselfassign) {
                $areaselfassign->delete();
                }
            $documentCV = DocumentCurrentVersion::where('accredlvl', $accreditation->id)->get();
            foreach ($documentCV as $documentCVs) {
                $documentCVs->delete();
                }
            $progresstable = Progress::where('accredlvlId', $accreditation->id)->get();
            foreach ($progresstable as $progresstables) {
                $progresstables->delete();
                }
                $instcomment = InstrumentComment::where('accredId', $accreditation->id)->get();
            foreach ($instcomment as $instcomments) {
                $instcomments->delete();
                }
    ...
    {
        $taskForces = AreaAssign::select('id', 'userId', 'areaId', 'parent', 'role', 'action', 'levelId')
    ->with([
        'userAssigned' => function ($query) {
            // Select only the fields we need from the related User model
            $query->select('id', 'first_name', 'last_name');
        },
        'instruments' => function ($query) {
            // Select only the fields we need from the related Instrument model
    ...
    ->get();
        ...
        // Return the task forces as a JSON response
        return response()->json([
            'success' => true,
