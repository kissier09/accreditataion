    ...
    public function store(Request $request)
{
    ...
    try {
        ...
        // Check the document type and set the document file
        if ($request->documentType == 'link') {
            $documentFile = $request->link;
        } else {
            // Get the file from TemporaryFile based on tempId
        ...
        // Create a new DocumentCurrentVersion
        ...
        // If document type is not 'link', process the file
        if ($request->documentType != 'link') {
            $this->processFile($file->folder, $file->file);
        }

        // Get the areaId for the instrument
        $areaTfc = $this->getArea($request->instrument)->last()->id;


        // If no assignment is found, default to the authenticated user
        $userId = Auth::id();

        // Prepare docuUpload data
        $docuUpload = collect([
            "userId" => $userId,
        ...
        // Log user activity for uploading the document
        $this->userLog($request->accredlvl, $document->id, $request->instrument, 'uploaded this document');

        // If the uploading user is not the assigned user, trigger DocumentUpload
        if ($userId != Auth::id()) {
